...
1. Install dependencies:
   `npm install`
2. Set `VITE_GEMINI_API_KEY` in `.env` only if you want browser-side Gemini fallback
3. Update branding in `config/brandConfig.ts` (store name, logo URL, banner URL, emojis, links, storage prefix)
4. Configure bot bridge vars in `.env`:
   `VITE_BOT_API_URL` (set to your OCI bot API URL, e.g. `https://api.robloxkeys.store`),
   `VITE_BOT_API_PREFIX` (default `/api/bot`),
   `VITE_BOT_API_KEY`,
   optional `VITE_BOT_API_KEY_HEADER`, `VITE_BOT_API_AUTH_SCHEME`, `VITE_BOT_API_TIMEOUT_MS`
5. Configure shop API vars in `.env`:
   `VITE_STORE_API_URL` (set to your OCI API URL),
