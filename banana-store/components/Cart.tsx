  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-[70] w-full max-w-md bg-[#0a0a0a] border-l border-white/10 shadow-2xl flex flex-col transform transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]">
        <div className="flex items-center justify-between border-b border-white/5 p-5 animate-slide-up sm:p-8">
        ...
        <div className="scrollbar-hide flex-1 space-y-5 overflow-y-auto p-4 sm:space-y-6 sm:p-8">
          {items.length === 0 ? (
            ...
            items.map((item, index) => (
              <div 
                key={item.id} 
                className="group flex gap-4 rounded-[24px] border border-white/5 bg-white/[0.02] p-4 transition-all animate-reveal hover:border-[#facc15]/20 sm:gap-5 sm:rounded-[32px] sm:p-5"
        ...
        {items.length > 0 && (
          <div className="border-t border-white/5 bg-black/40 p-5 backdrop-blur-md animate-slide-up sm:p-8">
            ...
            <button 
              onClick={onCheckout}
              className="group flex w-full items-center justify-center gap-3 rounded-3xl bg-[#facc15] py-5 text-xs font-black uppercase tracking-[0.24em] text-black shadow-[0_10px_40px_rgba(250,204,21,0.2)] transition-all active:scale-[0.98] hover:bg-yellow-300 sm:py-6 sm:tracking-[0.3em]"
            >
