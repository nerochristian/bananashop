...
# --- Legacy Helper Support for Existing Commands ---

class ComponentsV2:
    @staticmethod
    def create_container(title: Optional[str] = None, color: int = Colors.PRIMARY) -> 'ContainerBuilder':
        return ContainerBuilder(title, color)

    @staticmethod
    def create_section() -> 'SectionBuilder':
        return SectionBuilder()

class ContainerBuilder:
    def __init__(self, title: Optional[str] = None, color: int = Colors.PRIMARY):
        self.embed = discord.Embed(color=color)
        if title:
            self.embed.title = title
        self.components: List[discord.ui.View] = []
        self._fields: List[Dict[str, Any]] = []

    def set_description(self, description: str) -> 'ContainerBuilder':
        self.embed.description = description
        return self

    def add_section(self, section: 'SectionBuilder') -> 'ContainerBuilder':
    ...
    def add_field(self, name: str, value: str, inline: bool = True) -> 'ContainerBuilder':
        self.embed.add_field(name=name, value=value, inline=inline)
        return self
    
    def set_footer(self, text: str, icon_url: Optional[str] = None) -> 'ContainerBuilder':
        self.embed.set_footer(text=text, icon_url=icon_url)
        return self

    def set_image(self, url: str) -> 'ContainerBuilder':
        self.embed.set_image(url=url)
        return self
    
    def set_thumbnail(self, url: str) -> 'ContainerBuilder':
...
class SectionBuilder:
    ...
    def set_title(self, title: str) -> 'SectionBuilder':
        self.title = title
        return self

    def set_content(self, content: str) -> 'SectionBuilder':
        self.content = content
        return self
    
    def add_text_display(self, text: str, style: str = "normal") -> 'SectionBuilder':
        # "style" could map to markdown like bold, code block, etc.
        prefix = ""
        suffix = ""
        if style == "code":
            prefix = "`"
        ...
        new_content = f"{prefix}{text}{suffix}"
        if self.content:
            self.content += f"\\n{new_content}"
        else:
...
class MediaGalleryBuilder:
    # Discord embeds only support one main image. 
    # To have a "gallery", we can use multiple embeds pointing to the same url (hacky) 
    # or just set the main image.
    pass

# Helper Functions to match the TS interface
def create_container(title: Optional[str] = None, color: int = Colors.PRIMARY) -> ContainerBuilder:
    return ContainerBuilder(title=title, color=color)

def create_info_box(text: str, emoji: str = Emojis.INFO) -> str:
    return f"### {emoji} Info\\n> {text}"

def create_branded_header(title: str, sub: str, logo: str) -> discord.Embed:
    embed = discord.Embed(title=title, description=sub, color=Colors.PRIMARY)
    embed.set_thumbnail(url=logo)
    return embed

def create_stat_section(label: str, value: Any, emoji: str) -> str:
...
def create_feature_list(features: List[str], check_mark: str = Emojis.SUCCESS) -> str:
    return "\\n".join([f"{check_mark} {feature}" for feature in features])
